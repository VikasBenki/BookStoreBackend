    ...
    public class FeedbackRL :IFeedbackRL
    {
        ...
        public AddFeedback AddFeedback(AddFeedback addFeedback, int userId)
        {
            using (SqlConnection con = new SqlConnection(configuration["ConnectionStrings:BookStore"]))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("SP_Add_Feedback", con);
        ...
        public List<FeedbackResponse> GetAllFeedbacks(int bookId)
        {
            using (SqlConnection con = new SqlConnection(configuration["ConnectionStrings:BookStore"]))
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_GetAll_Feedback", con);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public FeedbackResponse ReadData(FeedbackResponse feedback, SqlDataReader rdr)
        {
            ...
            feedback.FullName = Convert.ToString(rdr["Full_Name"] == DBNull.Value ? default : rdr["Full_Name"]);
